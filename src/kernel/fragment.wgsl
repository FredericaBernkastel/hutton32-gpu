@fragment fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let parity = bool(simulation_buffer_tex[offset] >> 16u);
  let cell = (simulation_buffer_tex[offset] >> (u32(!parity) * 8u)) & 0xffu & (boundary * 0xffu);
  ...
  /*if (highlight_signals) {
    let offset = pixel.y * uniforms.simulation_dimm.x + pixel.x;
    let cell_diff = simulation_buffer_tex[offset];

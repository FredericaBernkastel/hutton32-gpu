...
fn output(c: u32,n: u32,s: u32,e: u32,w: u32) -> u32 { // what is the state of the cell we are pointing to?
  return 0u - u32(is_east(c)) & e
    | 0u - u32(is_north(c)) & n
    | 0u - u32(is_west(c)) & w
    | 0u - u32(is_south(c)) & s;
}
fn input(n: u32,s: u32,e: u32,w: u32) -> u32 { // what is the state of the excited cell pointing at us?
  return 0u - u32(is_east(w) && is_excited(w)) & w
    | 0u - u32(is_north(s) && is_excited(s)) & s
    | 0u - u32(is_west(e) && is_excited(e))  & e
    | 0u - u32(is_south(n) && is_excited(n)) & n;
}
...
fn is_crossing(n: u32,s: u32,e: u32,w: u32) -> bool {
	let n_inputs = u32(is_south(n)) + u32(is_east(w)) + u32(is_west(e)) + u32(is_north(s));
	let n_outputs = u32(is_TS(n) && !is_south(n))
	  + u32(is_TS(w) && !is_east(w))
	  + u32(is_TS(e) && !is_west(e))
	  + u32(is_TS(s) && !is_north(s));
	return n_inputs==2u && n_outputs==2u;
}

fn quiesce(c: u32) -> u32 {
  let cond1 = (c>=13u && c<=16u) || (c>=21u && c<=24u);
  let cond2 = c>=26u && c<=31u;
  return 0u - u32(cond1) & (c - 4u)
    | 0u - u32(cond2) & 25u
    | 0u - u32(!cond1 && !cond2) & c;
}
